...
void setup() {
  ...
  /*---DIGITAL INPUTS---*/
  pinMode(so  , INPUT);    			//Señal cuadrada para velocidad de la rueda
  ...
  //attachInterrupt(digitalPinToInterrupt(sw), blink, LOW);
  //attachInterrupt(digitalPinToInterrupt(so), blink, LOW);
...
void loop() {							//Loop principal del programa
  ...
  if( (tiempo_programa >= muestreo)){
    ...
    //detachInterrupt(digitalPinToInterrupt(so));
    ...
    //attachInterrupt(digitalPinToInterrupt(sw), blink, LOW);    	//Vuelve a habilitar las interrupciones para que funcione el programa
    //attachInterrupt(digitalPinToInterrupt(so), blink, LOW);
...
void calcular_capacidad(bool tabla){	//Calculo de capacidad restante de  la bateria y calculo por tabla
	...
	corriente_objetivo = (SOC * (tiempo_objetivo/60));
}
...
void debug (){							//Datos en puerto serial para debug
